...
In mathematical form, we want to find $Q* = argmax_Q P(Q\mid  O, \lambda)$.
...
In mathematical form, we want to find $\lambda* = argmax_\lambda P(O \mid \lambda)$.
...
So the Evaluation Problem asks: **What is the probability for this sequence to be generated from the current HMM?** or in other way: Calculate $P(O \mid \lambda)$.
...
So if we want to calculate the exact probability of $P(O \mid \lambda)$, we would have to sum over all possible sequences of hidden states that can generate the observation sequence $O$, then we will have:
$$P(O \mid \lambda) = \sum_{Q} P(O,Q \mid \lambda) $$
$$= P(O,Cold, Hot,Cold \mid \lambda) + P(O,Hot,Hot,Cold \mid \lambda)+ ...$$

Let's break down the math a little bit, we have:
$$P(O, Q) = P(O \mid Q) P(Q)$$
so:
$$P(O,Q \mid λ)=P(O \mid Q,λ)P(Q \mid λ)$$

1. For $P(Q \mid λ)$:

We have: $$P(Q \mid λ) =P(q_1, q_2, ...q_T)$$

$$= P(q_1) \cdot P(q_2 \mid q_1) \cdot P(q_3 \mid q_1, q_2) \cdot ... \cdot P(q_T \mid q_1, ..., q_{T-1})$$

And so for HMM:

$$P(Q \mid λ) = P(q_1, q_2, ...q_T \mid λ)$$
...
2. For $P(O \mid Q,\lambda)$ we have:
...
alpha_2(C) = (P(C). P(C \mid C) + P(H).P(C \mid H)) * P(1 \mid C)
...
In the formula, Q represents a random sequence of hidden states, and O represents the observations. The function argmax returns the value of Q that maximizes the probability $P(Q \mid O)$, not the value of the probability.
